...
ModelDefinition.prototype.query = function() {
  ...
  this.buildFillPlan(arguments.length >= 3 ? args[2] : {});
...
ModelDefinition.prototype.findAll = function(options) {
    this.buildFillPlan(options);
  return this.query(QueryGenerator.selectQuery(this.tableName, options, this), this, {fillPlan: options.fillPlan})
}
...
ModelDefinition.prototype.find = function(options) {
  ...
  var query = QueryGenerator.selectQuery(this.tableName, options, this)
  return this.query(query, this, {plain: true, fillPlan: options.fillPlan })
}

ModelDefinition.prototype.buildFillPlan = function(options)
{
    if (!options.fill || options.fillPlan)
        return;

    // Expand the requested list of fill fields into a tree structure.  Handles recursive field references such as A.B.C
    // Field names must match association names, otherwise they are ignored.
    var struct = { prefix: this.tableName, defn: this, attributes: this.attributes, children: {} };
    var fill = Array.isArray(options.fill) ? options.fill : [options.fill];
    fill.forEach(function(name) {
        name.split('.').reduce(function(struct, part) {
            if (!struct) return null;
            var child = struct.children[part];
            if (child) return child;
            var assoc = struct.defn.associations[part];
            if (!assoc || !assoc.joinStatement) return null;
            child = struct.children[part] = { prefix: struct.prefix+'_'+part, assoc: assoc, defn: assoc.target, attributes: assoc.target.attributes, children: {} };
            return child;
        }, struct);
    });


    // Filter list of attributes (if specified) into same structure, replacing default list of attributes at each node.
    if (options.attributes) {
        options.attributes.forEach(function(attr) {
            attr.split('.').reduce(function(struct, part, index, array) {
                if (!struct) return null;
                if (index < array.length - 1)
                    return struct.children[part];
                if (struct.attributes === struct.defn.attribute)
                    struct.attributes = {};
                struct.attributes[part] = '';
            }, struct);
        });
    }

    options.fillPlan = struct;
}

ModelDefinition.prototype.build = function(values, options, fillPlan) {
  var instance = new Model(values, Utils._.extend(this.options, {hasPrimaryKeys: this.hasPrimaryKeys}), fillPlan && fillPlan.prefix+'+')
    , self     = this
  ...
  if (fillPlan) Utils._.each(fillPlan.children, function(child, childName) { instance[childName] = child.defn.build(values, options, child);});
...
ModelDefinition.prototype.create = function(values, flags) {
  flags = flags || {}
  return this.build(values).save(flags)
};

ModelDefinition.prototype.__defineGetter__('identityAttribute', function() {
    var result;

    Utils._.detect(this.attributes, function(dataTypeString, attributeName) {
