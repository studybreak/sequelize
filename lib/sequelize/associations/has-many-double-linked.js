...
HasManyDoubleLinked.prototype.injectGetter = function() {
  ...
  var customEventEmitter = new Utils.CustomEventEmitter(function() {
    var where = {}
    where[self.definition.identifier] = self.instance[self.instance.definition.identityAttribute]
    ...
    self.definition.connectorModel.findAll({where: where}).on('success', function(associatedObjects) {
      ...
      if (ids.length == 0) {
        ...
        self.definition.target.findAll({where: self.definition.target.identityAttribute + ' in (' + ids.join(", ") + ')'})
